  ...
  def MDL(D, ParameterP, warm=False):
    ...
    while(True):
      try:
        ...
        DataSelected = DataFile[(DataFile[".id"] == "median")]
        DataSelected = DataSelected["I_new_sympt"].tolist()
        ReportedPP = DataSelected[1:46]
        ReportedPP = np.array(ReportedPP)
        
        DataSelected = DataFile[(DataFile[".id"] == "median")]
        DataSelected = DataSelected["I_new_asympt"].tolist()
        UnreportedPP = DataSelected[1:46]
        ...
        ModelCost3 = TimeSeriesCost(ParameterPP[1]*D[1:46] - ReportedP)
        DataCost = TimeSeriesCost(((D[1:46]-Reported[1:46])/(1.0-ParameterPP[1]))-(ReportedPP+UnreportedPP))
  ...
  def MDL_alpha(alpha, ParameterP):
    ...
    while(True):
      try:
        ...
        DataCost = TimeSeriesCost((((ReportedPP+UnreportedPP)-Reported[1:46])/(1.0-ParameterPP[1]))-(ReportedPP+UnreportedPP))
        MDLCost = ModelCost1 + ModelCost2 + ModelCost3 + DataCost
        break
      except:
        pass
