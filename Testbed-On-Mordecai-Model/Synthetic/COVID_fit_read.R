...
usable.cores      <- 36        ## Number of cores to use to fit
...
for (i in 5:nrow(variable_params)) {
  ...
  #preparing a warm start:
  read_params <- read.csv(file = 'parameter.csv')
  #print(read_params, quote = TRUE, row.names = FALSE)
  ...
  print("BETA:")
  print(beta_read)
  #print(beta_read,alpha_read,mu_read,delta_read,soc_dist_read)
...
write.csv(variable_params,file="parameter0.csv",quote=F,row.names = F) 