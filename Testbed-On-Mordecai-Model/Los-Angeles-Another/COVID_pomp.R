...
sir_step <- Csnippet("
                     ...
                     double dSE = rbinom(S, 1-exp(-beta0*(Ca*Ia/N + Cp*Ip/N + iso_m*Cm*Im/N + iso_s*Cs*Is/N)*dt)); 
                     double rateE[2];
                     double dE_all[2];
...
dmeas_deaths <- Csnippet("double tol = 1e-16;
                   if (ISNA(deaths)) {
                       ...
                       if (D_new < 6000) {                         
                           lik = 1.0*dpois(deaths, D_new + tol, 1);
                       }
                       else {
                           lik = - 10000000;
                       }
...
if (fitting) {
par_trans   <- parameter_trans(log = c("beta0", "alpha", "mu", "delta", "import_rate", "E_init", "Ia_init", "Ip_init", "Is_init", "Im_init", "Hr_init", "Hd_init", "R0_init", "D0_init"),    
