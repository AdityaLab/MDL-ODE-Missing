...
#################################################################################################
SEIRsimu <- function(pars, init_settings, num_periods = 1) {
  ...
  update_func <- function(stage_pars, states_old) {
    ...
    Onset_unreported_expect <- sample_P[2]
    ##
    return(c(S_new, E_new, P_new, I_new, A_new, H_new, R_new, Onset_expect, Onset_unreported_expect))
  }
  ## matrix for results
  states_mat <- matrix(0, length(days_to_fit), length(init_states) + 3)
  states_mat[, 1] <- days_to_fit
  colnames(states_mat) <- c("time", "S", "E", "P", "I", "A", "H", "R", "Onset_expect", "Onset_unreported_expect")
  ## evovle the system according to the discretized ODEs
  stage_start <- c(1)               # corresponding to dates Jan1, Jan10, Jan23, Feb2, Feb17
  stage_end <- c(length(days_to_fit))    # corresponding to dates Jan9, Jan22, Feb1, Feb16, the last day (could vary)
  ##
  ...
  if(num_periods == 1) {  ## total 5 periods: Jan 1-9, Jan 10-22, Jan 23-Feb 1, Feb 2-16, Feb 17-
