...
if __name__ == "__main__":
  ...
  ReportedP = np.array(ReportedP[:24])
  UnreportedP = np.array(UnreportedP[:24])
  ...
  def MDL(D, ParameterP, warm=False):
    ...
    os.system("Run_SEIR_main_analysis.R")
    ...
    ReportedPP = np.array(ReportedPP[:24])
    UnreportedPP = np.array(UnreportedPP[:24])
  ...
  res = minimize(fun=MDL_step2, x0=D_start, args=(D_start,ParameterP), method='nelder-mead', options={'maxiter':10, 'xtol':10000, 'ftol':1000})
