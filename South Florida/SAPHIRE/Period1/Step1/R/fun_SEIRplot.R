...
SEIRplot <- function(pars_estimate, file_name, init_settings, panel_B_R_ylim=4) {
  init_settings$days_to_fit <- 1:(60+60)
  # library(vioplot)
  # ##
  ...
  ##########################################   Panel A  ##########################################################
  estN_mat <- apply(pars_estimate, 1, function(x) SEIRsimu(pars = x, init_settings = init_settings, num_periods = 1)[, c("S", "E", "P", "I", "A", "H", "R", "Onset_expect", "Onset_unreported_expect")])
