...
if __name__ == "__main__":
  ...
  ReportedP = DataSelected[1:25]
  ReportedP = np.array(ReportedP)
    
  DataSelected = DataFile[(DataFile[".id"] == "median")]
  DataSelected = DataSelected["D_new_unreported"].tolist()
  UnreportedP = DataSelected[1:25]
  UnreportedP = np.array(UnreportedP)

  DataFile = pd.read_csv("parameter-original.csv")
  DataSelected = DataFile[(DataFile["sim_start"] == "2020-04-05")]
  ParameterP = [DataSelected["beta0est"].to_list()[0], DataSelected["alphaest"].to_list()[0], DataSelected["alpha1est"].to_list()[0]]
  ...
  def MDL(D, ParameterP, warm=False):
    ...
    while(True):
      try:
        ...
        ModelCost3 = TimeSeriesCost(ReportRate*D[1:25] - ReportedP)
        DataCost = TimeSeriesCost(((D[1:25]-Reported[1:25])/(1.0-ReportRate))-(ReportedPP+UnreportedPP))
  ...
  def MDL_alpha(alpha, ParameterP):
    ...
    while(True):
      try:
        ...
        DataCost = TimeSeriesCost((((ReportedPP+UnreportedPP)-Reported[1:25])/(1.0-ReportRate))-(ReportedPP+UnreportedPP))
        MDLCost = ModelCost1 + ModelCost2 + ModelCost3 + DataCost
        break
      except:
        pass
