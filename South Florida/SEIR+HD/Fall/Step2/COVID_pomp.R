...
sir_init <- Csnippet("
                     ...
                     double Ia0 = rpois(Ia_init) + 1; 
                     double Ip0 = rpois(Ip_init) + 1; 
                     double Is0 = rpois(Is_init) + 1; 
                     double Im0 = rpois(Im_init) + 1; 
                     double Hr0 = rpois(Hr_init) + 1; 
                     double Hd0 = rpois(Hd_init) + 1; 
                     double R0 = rpois(R0_init) + 1; 
                     double D0 = rpois(D0_init) + 1; 
                     S = N-E0-Ia0-Ip0-Is0-Im0-Hr0-Hd0-R0-D0;
                     E = E0;
                     Ia = Ia0;
                     Ip = Ip0;
                     Is = Is0;
...
dmeas_deaths <- Csnippet("double tol = 1e-16;
                   ...
                   if (ISNA(cases)) {
                       ...
                       if (D_new_reported < 5000) {                         
                           lik = lik + 0.05*dpois(cases, D_new_reported + tol, 1);
                       }
                       else {
                           lik = lik - 10000000;
                       }
                   ...
                   if (ISNA(uncases)) {
                       ...
                       if (D_new_unreported < 495000) {                         
                           lik = lik + 0.005*dpois(uncases, D_new_unreported + tol, 1);
                       }
                       else {
                           lik = lik - 10000000;
                       }
...
if (fit.E0) {
par_trans   <- parameter_trans(log = c("beta0", "alpha", "alpha1", "import_rate", "E_init", "Ia_init", "Ip_init", "Is_init", "Im_init", "Hr_init", "Hd_init", "R0_init", "D0_init"),
...
if (fit.E0) {
par_trans   <- parameter_trans(log = c("beta0", "alpha", "alpha1", "E_init", "Ia_init", "Ip_init", "Is_init", "Im_init", "Hr_init", "Hd_init", "R0_init", "D0_init"))
param_names <- c(
   "beta0"
  , "alpha1"
  , "Ca", "Cp", "Cs", "Cm"
