...
if __name__ == "__main__":
  ...
  ReportedP = np.array(ReportedP[:16])
  UnreportedP = np.array(UnreportedP[:16])
  ...
  def MDL(D, ParameterP, warm=False):
    ...
    if(True):
      if(True):
        ...
        ReportedPP = np.array(ReportedPP[:16])
        UnreportedPP = np.array(UnreportedPP[:16])
