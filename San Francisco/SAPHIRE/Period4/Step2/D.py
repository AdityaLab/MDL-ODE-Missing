...
if __name__ == "__main__":
  ...
  ReportedP = np.array(ReportedP[:36])
  UnreportedP = np.array(UnreportedP[:36])
  ...
  def MDL(D, ParameterP, warm=False):
    ...
    if(True):
      if(True):
        ...
        ReportedPP = np.array(ReportedPP[:36])
        UnreportedPP = np.array(UnreportedPP[:36])
        ...
        ModelCost3 = TimeSeriesCost(ReportRate*D[1:37] - ReportedP)
        DataCost = TimeSeriesCost(((D[1:37]-Reported[1:37])/(1.0-ReportRate))-(ReportedPP+UnreportedPP))
