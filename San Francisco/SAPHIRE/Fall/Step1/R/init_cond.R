generate_init_condi <- function(r0,
  ...
  ##
  E0 <- sum(realData[(jan1_idx+round(Dp)):(jan1_idx+round(Dp)+round(De)-1),1]) / r0 ## Jan 3-5 for De=2.9 and Dp=2.3
  # E0 <- (40 + 23 + 47) / r0                                              
  P0 <- sum(realData[jan1_idx:(jan1_idx+round(Dp)-1),1]) / r0                     ## Jan 1-2 for Dp=2.3
  # P0 <- (41 + 34) / r0
  I0 <- sum(realData[0:(jan1_idx),1])                             ## Dec 29-31 for Di=2.9
  # I0 <- 11 + 13 + 10                                     
  A0 <- I0 * (1 - r0) / r0
