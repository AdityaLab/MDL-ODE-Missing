...
if __name__ == "__main__":
  ...
  ReportedP = np.array(ReportedP[:42])
  UnreportedP = np.array(UnreportedP[:42])
  ...
  def MDL(D, ParameterP, warm=False):
    ...
    ReportedPP = np.array(ReportedPP[:42])
    UnreportedPP = np.array(UnreportedPP[:42])
  ...
  res = minimize(fun=MDL_step2, x0=D_start, args=(D_start,ParameterP), method='nelder-mead', options={'maxiter':10, 'xatol':1e3, 'fatol':100})
