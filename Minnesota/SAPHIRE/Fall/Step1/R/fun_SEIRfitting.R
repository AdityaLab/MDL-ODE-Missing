...
default_pars_density <- function(pars) {
  ...
  ## r1
  r1 <- pars[2]
  d_vec[2] <- dbeta(r1, beta_shape1, beta_shape2, log = T)
  ##
  return(sum(d_vec))
}
...
default_pars_sampler <- function(n = 1) {
  ...
  ## r1 
  r1 <- rbeta(n, beta_shape1, beta_shape2)
  s_vec[, 2] <- r1
  return(s_vec)
}
...
SEIRfitting=function(init_sets_list, 
  ...
  loglh_func <- function(pars){
    ...
    ypred_reported <- ypred[, "Onset_expect"]
    ypred_unreported <- ypred[, "Onset_unreported_expect"]
    # meant to suppress warnings when ypred is negative
    suppressWarnings(p_reported <- dpois(onset_obs, ypred_reported))
    suppressWarnings(p_unreported <- dpois(onset_unreported_obs, ypred_unreported))

    if((any(p_reported == 0) || any(is.nan(p_reported))) || (FALSE || any(is.nan(p_unreported)))){
      logL <- -Inf
    }else{
      logL <- sum(log10(p_reported))
      if (any(p_unreported == 0)) {
        ...
        logL = logL + 0.01*sum(log10(p_unreported))
      }
      else {
        logL = logL + 0.01*sum(log10(p_unreported))
      }
