...
if __name__ == "__main__":
  ...
  ReportedP = np.array(ReportedP[:32])
  UnreportedP = np.array(UnreportedP[:32])
  ...
  def MDL_alpha(alpha, ParameterP):
    ...
    ReportedPP = np.array(ReportedPP[:32])
    UnreportedPP = np.array(UnreportedPP[:32])
    ...
    DataCost = TimeSeriesCost((((ReportedPP+UnreportedPP)-Reported[1:33])/(1.0-ReportRate))-(ReportedPP+UnreportedPP))
