...
if __name__ == "__main__":
  ...
  ReportedP = np.array(ReportedP[:35])
  UnreportedP = np.array(UnreportedP[:35])
  ...
  def MDL(D, ParameterP, warm=False):
    ...
    ReportedPP = np.array(ReportedPP[:35])
    UnreportedPP = np.array(UnreportedPP[:35])
    ...
    ModelCost3 = TimeSeriesCost(ReportRate*D[1:36] - ReportedP)
    DataCost = TimeSeriesCost(((D[1:36]-Reported[1:36])/(1.0-ReportRate))-(ReportedPP+UnreportedPP))
