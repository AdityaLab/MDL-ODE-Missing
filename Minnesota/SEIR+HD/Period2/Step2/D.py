  ...
  def MDL(D, ParameterP, warm=False):
    ...
    while(True):
      try:
        ...
        ModelCost3 = TimeSeriesCost(ReportRate*D[1:34] - ReportedP)
        DataCost = TimeSeriesCost(((D[1:34]-Reported[1:34])/(1.0-ReportRate))-(ReportedPP+UnreportedPP))
  ...
  def MDL_alpha(alpha, ParameterP):
    ...
    while(True):
      try:
        ...
        DataCost = TimeSeriesCost((((ReportedPP+UnreportedPP)-Reported[1:34])/(1.0-ReportRate))-(ReportedPP+UnreportedPP))
        MDLCost = ModelCost1 + ModelCost2 + ModelCost3 + DataCost
        break
      except:
        pass
